...
window.addEventListener("load", function() {
    ...
    if (HTML.classList.contains("toc")) {
        ...
        toc.innerHTML = `<div class="toc-row"><a onclick="scrollToTop()" style="cursor: pointer;">(Top)</a>${ xButtonSvg }</div>` + headings.slice(1).map ( heading => `<div class="toc-row ${ heading.tagName.toLowerCase() }"><a href="#${ heading.id }">${ heading.innerHTML }</a></div>` ).join("");
